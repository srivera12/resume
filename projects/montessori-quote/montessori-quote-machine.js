// import { montessoriQuotes } from './data.js';
...
const montessoriQuotes = [
  ...
  {
    quote:
      '“Praise, help, or even a look, may be enough to interrupt him, or destroy the activity….  The great principle which brings success to the teacher is this: as soon as C has begun, act as if the child does not exist.”',
    book: 'The Absorbent Mind',
  ...
  },
  {
    ...
    book: 'The Child, Society and the World (Unpublished Speeches and Writing)',
    topic: ['Concentration', 'The Teacher', 'Development'],
  },
  {
    quote:
...
// creating quote select by topic
function createTopicSelect() {
  const topicList = [];
  for (let i = 0; i < montessoriQuotes.length; i++) {
    const topics = montessoriQuotes[i].topic;
  ...
  for (let i = 0; i < topicList.length; i++) {
    ...
    topicOption.setAttribute('value', topicList[i]);
    topicSelect.appendChild(topicOption);
  }
}
createTopicSelect();

// select by quote topic
function getQuotesTopic() {
  quotesByTopic.textContent = '';
  const value = topicSelect.value;
  for (let i = 0; i < montessoriQuotes.length; i++) {
    if (montessoriQuotes[i].topic.includes(value)) {
      let quote = document.createElement('blockquote');
      quote.textContent = montessoriQuotes[i].quote;
      quote.classList.add('quote-text');
      quotesByTopic.appendChild(quote);
    }
  }
}
topicButton.addEventListener('click', getQuotesTopic);
